...
export function ReportTemplate({ data, isPdfGeneration = false }) {
    ...
    return (
        <>
            {/* CSS for print - footer on every page */}
            {isPdfGeneration && (
                <style>{`
                    @media print {
                        @page {
            ...
            <div className={`bg-white text-slate-900 font-sans text-sm leading-tight ${isPdfGeneration ? 'report-content' : 'p-6 md:p-12 max-w-[210mm] mx-auto min-h-[297mm]'}`}>
                ...
                {/* Footer - no final do documento para web */}
                {!isPdfGeneration && (
            ...
            {/* Footer fixo para PDF - aparece em todas as páginas */}
            {isPdfGeneration && (
                <div className="report-footer-fixed" style={{
                    position: 'fixed',
                    bottom: 0,
                    left: 0,
                    right: 0,
