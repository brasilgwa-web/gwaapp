...
import { getStatusColor } from '@/components/analysisUtils';
...
export default function ReportView() {
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            // 1. Get Visit first to know client_id
            ...
            // 2. Fetch Client and Locations Manually
            ...
            // We return them here as an array to match previous destructuring logic if needed, 
            // but the code below expects [clientsArray, primaryLocationArray, allLocationsArray] ?
            // No, the previous code returned [clients, primaryLocations, allLocations] where specific ones were arrays of 1.
            // Let's adjust the downstream variable assignment or just return specific objects since we are replacing the Promise.all

            // Actually, let's keep the variable names stable for the next block
            // The previous block was:
            // const [clients, primaryLocations, allLocations] = await Promise.all([...])
            // and then: const client = clients[0];

            // So we should return arrays or just assign directly. 
            // I will assign directly below, so I need to change how the next lines consume them.
            // But wait, the ReplaceFileContent targets the Promise.all block.
            // I will replace it with the definitions of 'client', 'primaryLocation', 'allLocations' directly.

            // However, step 56-57 (in my mind) uses them.
            // Let's look at lines 71-72: const client = clients[0]; const primaryLocation = primaryLocations[0];

            // So to minimize diff, I'll return arrays of [data] or [].

            // Duplicate declarations removed

            // 3. Fetch LocationEquipments specifically for these locations (to avoid limit issues)
            ...
            // Removed redundant client/primaryLocation declarations since they are defined above from manual fetch results
                ...
                const equipmentsWithTests = allLocationEquipments
                    ...
                    .map(le => {
                        ...
                        const testsWithResults = tests.map(test => {
                            // Robust finding logic: match test_id AND (instance_id OR catalog_id)
    ...
    return (
        <div className="bg-slate-50 min-h-screen p-4 md:p-8 print:p-0 print:bg-white">
            <style>{`
                @media print {
            ...
            <div className="w-full md:max-w-[210mm] mx-auto mb-6 flex flex-col md:flex-row justify-between gap-4 print:hidden">
            ...
            {/* A4 Page Container - Responsive Print Width */}
            <div className="w-full md:max-w-[210mm] mx-auto bg-white shadow-lg print:shadow-none print:w-[210mm] print:max-w-none p-4 md:p-[15mm] min-h-[297mm] print:min-h-0 print:m-0 print:overflow-visible overflow-hidden">
                ...
                {/* Info Block */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 text-sm border rounded-lg p-4 bg-slate-50 print:bg-white print:border-slate-300">
                    <div>
                    ...
                    <div className="text-left md:text-right">
                        ...
                        <p><span className="font-semibold">Data:</span> {format(new Date(visit.visit_date), "d 'de' MMMM, yyyy", { locale: ptBR })}</p>
                        <p><span className="font-semibold">Local:</span> {primaryLocation?.name}</p>
                        <p className="break-all"><span className="font-semibold">Técnico:</span> {visit.technician_email || 'Técnico WGA'}</p>
                ...
                <div className="mb-8 space-y-6">
                    ...
                    {fullReportStructure.length === 0 && <p className="text-slate-500 italic">Sem equipamentos ou medições configuradas.</p>}
                </div>
